...
def apply_median(image: np.ndarray, window_size: int = 5) -> np.ndarray:
    ...
    # Log input
    logger.info(
        f"Median filter - Input shape: {image.shape}, dtype: {image.dtype}, "
    ...
    try:
        ...
        logger.warning(
            "Insufficient memory for vectorized median, using loop-based fallback"
        )
    ...
    logger.info(
        f"Median filter - Output shape: {result.shape}, Processing time: {elapsed_time:.4f}s"
    )
