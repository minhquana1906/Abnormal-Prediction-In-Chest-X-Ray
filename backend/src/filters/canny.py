...
def apply_canny(
    ...
    # Log input
    logger.info(
        f"Canny filter - Input shape: {image.shape}, dtype: {image.dtype}, "
    ...
    if auto_threshold and (low_threshold is None or high_threshold is None):
        ...
        logger.info(
            f"Canny - Auto-computed thresholds: low={low_threshold}, high={high_threshold}"
        )
    else:
        ...
        logger.info(
            f"Canny - Manual thresholds: low={low_threshold}, high={high_threshold}"
        )

    # Step 5: Double thresholding
    strong_edges, weak_edges = _double_threshold(
    ...
    logger.info(
        f"Canny filter - Output shape: {result.shape}, "
...
def _gaussian_blur(
    image: np.ndarray, sigma: float = 1.4, kernel_size: int = 5
) -> np.ndarray:
...
def _auto_threshold(
    magnitude: np.ndarray, low_percentile: float = 50, high_percentile: float = 85
) -> tuple:
    """
    ...
    if high_threshold <= low_threshold:
        ...
        logger.warning(
            f"Auto-threshold: high <= low, adjusted high to {high_threshold}"
        )
    ...
    # Log statistics
    logger.debug(
        f"Auto-threshold - Gradient stats: "
