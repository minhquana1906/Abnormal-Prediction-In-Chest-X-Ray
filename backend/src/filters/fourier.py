"""Fourier transform visualization filter - using NumPy FFT."""
...
def apply_fourier(image: np.ndarray) -> np.ndarray:
    ...
    logger.info(f"Fourier - Magnitude range: [{magnitude.min():.2f}, {magnitude.max():.2f}]")
    ...
    logger.info(f"Fourier transform - Output shape: {result.shape}, "
