"""Discrete Cosine Transform (DCT) filter - using NumPy-based implementation."""
...
def apply_dct(image: np.ndarray) -> np.ndarray:
    ...
    logger.info(f"DCT - Output shape: {result.shape}, "
    ...
    for k in range(n):
        for i in range(n):
            if k == 0:
                ...
                # AC components
                matrix[k, i] = np.sqrt(2.0 / n) * np.cos(np.pi * k * (2 * i + 1) / (2 * n))
