...
def apply_histogram_equalization(image: np.ndarray) -> np.ndarray:
    ...
    # logger.info(
    #     f"Histogram - Min value: {np.min(image)}, Max value: {np.max(image)}, "
    ...
    if total_pixels - cdf_min == 0:
        logger.warning(
            "CDF normalization: denominator is zero, returning original image"
        )
        return image
    ...
    # logger.info(
    #     f"Histogram equalization - Output shape: {result.shape}, "
