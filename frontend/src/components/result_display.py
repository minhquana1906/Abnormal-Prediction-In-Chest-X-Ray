...
def render_single_result(result: Dict[str, Any], result_number: int):
    ...
    with st.container(border=True):
        ...
        st.caption(f"**Thời gian:** {processing_time_ms} ms ({processing_time_ms / 1000:.3f}s)")
...
def render_side_by_side_comparison(
    ...
    st.caption(f"⏱️ Thời gian xử lý: {processing_time_ms} ms ({processing_time_ms / 1000:.3f}s)")
...
def render_download_all_button(results: List[Dict[str, Any]], original_filename: str):
    ...
    if st.button("📥 Tải xuống tất cả ảnh đã xử lý (ZIP)", use_container_width=True):
